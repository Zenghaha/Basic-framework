...
public class DateUtil {
	...
	public static Date getYesterDay(int day){
	...
	public static Date parse(String dateStr) {
		return DateUtil.parse(dateStr, null);
	}
	...
	public static int getSubtractSecond(Date sDate, Date eDate){
		Calendar calendar = Calendar.getInstance(); 
		calendar.setTime(sDate); 
		long timethis = calendar.getTimeInMillis();  
		...
		long timeend = calendar2.getTimeInMillis(); 
		long thedaymillis = timeend-timethis; 
		return Integer.valueOf(String.valueOf(thedaymillis/1000));
	}
   ...
   public static Date msToDate(Long ms,String format) throws ParseException{
	 ...
	 * @return
	 */
    public static List<String> getDateStrList(String startDateStr, String inPattern, String endDateStr, String outPattern) {
        List<String> list = new ArrayList<>();
        Date startDate = DateUtil.parse(startDateStr, inPattern);
        Date endDate = DateUtil.parse(endDateStr, inPattern);
        while (startDate.before(endDate) || startDate.equals(endDate)) {
            list.add(DateUtil.format(startDate, outPattern));
            startDate = getNextDate(startDate);
        }
        return list;
    }
     ...
     * @return
     */
    public static List<String> getDateStrList(Date startDate, Date endDate,String outPattern) {
		List<String> list = new ArrayList<>();
		while (startDate.before(endDate) || startDate.equals(endDate)) {
			list.add(DateUtil.format(startDate, outPattern));
			startDate = getNextDate(startDate);
    ...
    public static List<String> getMonths() {
		...
		for (int i = 0; i < 12; i++) {
			...
			list.add(cal.get(Calendar.YEAR)+"-"+month);
